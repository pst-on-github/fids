...
static int fnmatch_filespec_list(char *speclist, char *fname)
{
    ...
    while (*speclist) /* Parse "path_1:path_2:..." */
    {
        ...
        while (speclist[strl] && (speclist[strl] != ':'))
            strl++;
...
char *dir_name;
{
    ...
    printf("%sreading directory... !%s%s", A_MESSAGE, A_NORMAL, DEL_EOL);
    fflush(stdout);
    ...
    while ((p_dirent = readdir(dp)) != (struct dirent *)0)
    {
...
#elif defined QNX6
        {
            ...
            for (extra                                 = _DEXTRA_FIRST(p_dirent);
                 _DEXTRA_VALID(extra, p_dirent); extra = _DEXTRA_NEXT(extra))
            {
    ...
    if ((p_ls = (struct FIDS **)calloc(num, sizeof(p_ls))) == (struct FIDS **)0)
        syserr("fids_readdir:calloc");
