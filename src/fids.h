...
GLOBAL char *
    KPAD_OFF GLOBAL_INIT("\033[?1l\033>"); /* cursor keypad mode normal */
...
#define FIDS_TERM_NAMES                              \
    {                                                \
        {VT100_TERM, VT100_GRAF_ON, VT100_GRAF_OFF}, \
        {                                            \
...
#define FIDS_TERM_ATTR                                      \
    { /* Used attributes for 'vt100' */                     \
...
#define A_NORMAL (Term_Attr[TermGrix][0]) /* Normal display (all attributes) \
                                           */
#define A_WORKDIR \
    (Term_Attr[TermGrix][1]) /* Working directory display in the first line */
#define A_NOFMFILES \
    (Term_Attr[TermGrix][2])                  /* n of m files display in the second line */
...
#define A_ACTUAL         \
    (Term_Attr[TermGrix] \
              [6]) /* Currently selected file (active page, if splited) */
#define A_OTHER          \
    (Term_Attr[TermGrix] \
              [7])                             /* Currently selected file (passive page, if splited) */
...
#define A_ACTFILEDATA \
    (Term_Attr[TermGrix][18]) /* Actual file data display (data) */
#define A_ACTFILENAME \
    (Term_Attr[TermGrix][19])                   /* Actual file data display (name) */
...
#else
#define FIDS_NAME_LEN_MAX \
    (MAXNAMLEN > 64 ? (MAXNAMLEN > 256 ? 256 : MAXNAMLEN) : 64)
#endif
