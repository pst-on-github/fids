...
static  int dis_c = 0;             /* startindex of display of cmd_line */
	...
	else if( dlen < max_c && dnum >= (TermCols-1) )
	...
	if( dnum < 0 || dnum > (TermCols-1) || scp == 'f' )
	...
	if( scp == 'b' )
	{
	  ...
	  printf("%s%s%s",A_CMDLMODE,
	    insert_mode?"[ insert ]":"[exchange]",A_NORMAL);
...
{
	if ( actlr == 'r' ) return( cmd_m_ird() );
	else                return( cmd_m_ild() );
}

/************************************************************************/
static int cmd_m_nwd( void )         /* get directory not active side   */
{
	if ( actlr == 'l' ) return( cmd_m_ird() );
	else                return( cmd_m_ild() );
}
...
static int cmd_m_cpy( void )         /* copy actual file to other side  */
{
	...
	if ( !p_ls[acti]->f_inode ) ins_nam("@ ",'\0');    /* if marked */
	else ins_nam( p_ls[acti]->f_name, '1' );
...
static char *sub_cmd( char *p_s ) /* command substitution execution via pipe */
{
 ...
 static char p_ret[PRET_LEN];
	...
	*p_ret = '\0';
	if ( (pp = malloc( (unsigned) strlen(p_cmdl[0]) + 2 +
		strlen(EXEC_SH)+strlen(STDINOFF)+strlen(STDERR21)) ) == NULL )
		syserr("fids:malloc");

	gotoxy(1,TermLines-1);
	printf("%ssubstituting...%s%s",
	...
	*p_i = '\0';
	...
	sig_nocatch();
	if( !(fp = popen(pp,"r")) )     syserr("popen failed");
	for(p_i=p_ret, i=sizeof(p_ret)-1; (*p_i=fgetc(fp)) != (char)EOF; )
	{
	    if( *p_i < ' ' ) *p_i = ' ';    /* if CR etc. make it to blank  */
	    if ( i ) { p_i++; i--; }        /* read always the hole pipe    */
	}
	*p_i-- = '\0';
        while( p_i > p_ret && *p_i == ' ' )
          *p_i-- = '\0';
...
/**********  end  C O M M A N D     S U B S T I T U T I O N  ************/
    ...
    if (!mark_flag)
    {
      ...
      for (i=0; *cmd_table[i].cmd_name ; i++)
      {
        ...
        if( strlen(p_cmdl[1]) < m ) m--;   /* (# - ' ') */
        if ( !strncmp(p_cmdl[1],cmd_table[i].cmd_name,m) )
...
/**************** L O C A L E S       E N V I R O N M E N T *************/
      ...
      for( p=p_cmdl[1] + 1; *p ; p++ )    /* search for 'name=value' */
      {
        ...
        {
           char *penv;
           if( (penv=malloc((unsigned)(strlen(p_cmdl[1])+1))) == NULL)
             syserr("fids:malloc");
...
int rc;
{
	PF_label(1);
	restore_cursor();
	if( rc == 0 ) mesg_line();             /* restore the mesg_line */
	quit = TRUE;                           /* set leave flag        */
	return(rc);
}
...
static int cmd_rows( void )                            /* toggle FileLstRows */
{
        switch( FileLstRows )
        {
          case 4 :  FileLstRows = 2;
                    break;
          case 2 :
          default : FileLstRows = 4;
                    break;
        }
...
#define FIDS_SET_CMD_cscmd      "cscmd"       /* cmd for setting SUBST_CMD_CHAR */
	...
	pp = strchr( p_cmdl[1], ' ' );  /* jump after the 'fids_set' */
	  ...
	  printf("Actual values : '%s'= %s%d%s , '%s'= %s%d%s , '%s'= %s%c%s , '%s'= %s%c%s , '%s'= %s%c%s%s",
	         FIDS_SET_CMD_alarm, A_SETVALUES, time_def, A_NORMAL,
	         FIDS_SET_CMD_rows, A_SETVALUES, FileLstRows, A_NORMAL,
	         FIDS_SET_CMD_ssoe, A_SETVALUES, FS_save_flag?'y':'n', A_NORMAL,
	         FIDS_SET_CMD_csev, A_SETVALUES, SUBST_ENV_CHAR, A_NORMAL,
	         FIDS_SET_CMD_cscmd, A_SETVALUES, SUBST_CMD_CHAR, A_NORMAL,
	         DEL_EOL);
	  fflush(stdout);
	...
	pp = strchr( p1, ' ' );         /* jump after the 1. token */
        ...
        else if( strcmp( p1, FIDS_SET_CMD_csev ) == 0 )    /* char for subst. env variable */
        {
          if( p2 == NULL )
        ...
        else if( strcmp( p1, FIDS_SET_CMD_cscmd ) == 0 )   /* char for subst. command */
        {
          if( p2 == NULL )
	  ...
	  {
	    char *penv;
	    if( (penv=malloc((unsigned)(strlen(p1)+strlen(p2)+2))) == NULL)
	      syserr("fids:malloc");
        ...
        {
	  error( "","" );			     /* set the error flag ! */
	  printf("Actual value for '%s' = %s%s%s%s", p1, A_SETVALUES,old_val,A_NORMAL,DEL_EOL);
	  fflush(stdout);
	  return(-1);
        }
...
{
	gotoxy(1,TermLines-1);
	printf("Update interval time = xx sec  < CURSOR UP/DOWN: change,  else go back >%s",
				DEL_EOL);
	...
	for (;;)
	{
		...
		case CURK('A'):
		case CTRL('^'):
		                  if ( time_def < CLK_RATE ) time_def++;
				break;
		case CURK('B'):
		case 28 /*^\*/:
		                  if ( time_def > 2        ) time_def--;
				break;
		case CTRL('E'):
		case CTRL('F'):   time_def = CLK_RATE;
				break;
		case CTRL('X'):
		case CTRL('L'):   time_def = 2;
				break;
		default:        restore_cursor();
				mesg_line();
				return(0);
	    }
	}
...
static int cmd_comp( void )                         /* compare 2 directories */
{
    ...
    gotoxy(1,TermLines-1);
    printf("%sComparing all files...%s%s",
        A_MESSAGE,A_NORMAL,DEL_EOL);
    fflush(stdout);
	...
	*p_ret = '\0';
        ...
        while( (*p_i=fgetc(fp)) != (char)EOF )
	{
	    if( *p_i < ' ' ) *p_i = ' ';    /* if CR etc. make it to blank  */
            ...
            {
              if ( len ) { p_i++; len--; }      /* read always the hole pipe    */
            }
        }
        pclose(fp);
...
static int cmd_pipe( void )     /* intern sh command execution via pipe */
{
	...
	*p_ret = '\0';
	if ( (pp = malloc( (unsigned) strlen(p_cmdl[1]) + 2 +
		strlen(EXEC_SH)+strlen(STDINOFF)+strlen(STDERR21)) ) == NULL )
		syserr("fids:malloc");
	...
	if( !(fp = popen(pp,"r")) )     syserr("popen failed");
        FidsPcloseOnInterrupt = fp;
        setraw(1);                       /* make it interruptable ! */
	for(p=p_ret, i=sizeof(p_ret)-1; (*p=fgetc(fp)) != (char)EOF; ) /*lint !e789 Assigning address of auto (p_ret) to static (p) */
	{
	    if( *p < ' ' ) *p = ' ';    /* if CR etc. make it to blank  */
	    if ( i ) { p++; i--; }      /* read always the hole pipe    */
	}
	*p = '\0';
	pclose(fp);
	FidsPcloseOnInterrupt = NULL;
...
{                                       /*         and edit old line    */
	...
	for(;;)
	{
	    ...
	    if ( quit ) return(rc);     /* leave the cmd_mode           */
	    ...
	    {
		if( ( c == cmd_mod[i].cmd_char ) ||
		    ( c == cmd_mod[i].cmd_cchar ) )
		{
