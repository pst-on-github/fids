...
static  char *term;                     /* terminal type from $TERM     */
	...
	BOOLEAN GRAF_VT100 = TRUE;      /* switch for vt100 Graphicmode */
        ...
        char *Term_Attr_Str[ TERM_NAME_NUM ][ TERM_ATTR_STRING_NUM ] = FIDS_TERM_ATTR_STRINGS;
...
/*                                                                      */
	...
	if ( argc >= 2 )
	{
	      ...
	      !strcmp( argv[1], "help" ) )
	  {
	    ...
	    fprintf( stderr,"Usage: fids [-] [ -l name ] [ -e VAR ] [-s] [-g] [ dir ]\n\n" );
	    fprintf( stderr,"\t-        : %s=uid\n", llsswe );
	    fprintf( stderr,"\t-l name  : %s=name\n", llsswe );
	    fprintf( stderr,"\t-e VAR   : %s from env. $VAR\n", llsswe );
	    fprintf( stderr,"\t-s       : don't save the actual settings on exit\n" );
	    fprintf( stderr,"\t-g       : don't use the graphic vt100 symbols\n" );
	    fprintf( stderr,"\tdir      : do 'cd dir first'\n\n" );
	    fprintf( stderr,"\tIf ![-s] : On exit the actual user settings are stored with\n" );
	    fprintf( stderr,"\t           \"$HOME/%s_'extension'\". (default extension=uid)\n",FS_FILE );
	    exit(-1);
	  }
	}
	...
	if ( (z1 = getenv("PS1"))==NULL) z1 = "$ ";
	if ( (ps1 = malloc((unsigned)strlen(z1)+strlen(FIDS_PS1)+1) ) == NULL )
		syserr("fids:malloc");
	strcpy(ps1,FIDS_PS1);
	strcat(ps1,z1);
	if ( putenv(ps1) ) syserr("fids.putenv");
	...
	makescreen();                        /*     make the screen new */
        ...
        if( ( TermLines < TERM_MIN_LINES ) ||
            ( TermCols < TERM_MIN_COLS ) )
        {
          cmd_exit();
        }
	...
	if ( !cready() )
	{
		if ( !stat_list    ) stat_list = fids_list('.','b');
		if ( !stat_fidsnew ) stat_list = fidsnew('.');
	}
...
static int cmd_sort( void )                           /* command execution for sort */
{
	...
	if (actlr == 'r') smod_old = smodr;
	      else        smod_old = smodl;

	save_cursor();
	gotoxy(1,TermLines-1 );
	printf(" %ssort files?%s  %sn%s%same%s, %st%s%s=[.*]%s, %ss%s%size%s, %su%s%sid%s, %sg%s%sid%s",
		   A_SORT,A_NORMAL,
		   A_SORTKEY,A_NORMAL,A_SORT,A_NORMAL,   /* name */
...
char *dir_name;
{
	...
	while(	( chdir( dir_name )	   == ERR ) ||
		( stat( dir_name, &stdir ) == ERR ) ||
		( access( "." , 04 )	   == ERR ) )
	{
...
char what;
{
	...
	if ( access(".",04) == ERR )
	{
		...
		if( cmd_cdu( p_cwd ) == ERR )
		      cmd_exit();
        	if (what != 's') stat_list = fidsnew('b');  /* s=switch cwd */
		return(-1);
	}

	strcpy(dir_f_n,"..");
	if ( what == 'f' && !strcmp(dir_name,"..") && strlen(p_cwd) > 1 )
	{                                         /* get name of subdir */
...
static void continue_or_abort( void )
{
  ...
  printf( " ==> %sHit any key to restart fids! ('%c' aborts)%s%s\n",
          A_MESSAGE, ABORT_CHAR, A_NORMAL, DEL_EOL );
#else
  printf( " ==> %sHit any key to restart fids! ('%c' aborts)%s%s",
          A_MESSAGE, ABORT_CHAR, A_NORMAL, DEL_EOL );
#endif
...
{                                       /* if 2dirs then unsplit        */
	...
	if (scrn == 1)                  /* activ = 'l' if scrn == 1     */
	{
		...
		p_lsr = (struct FIDS **)calloc((unsigned)(numd+numf),
						sizeof(p1));
		if (!p_lsr) syserr("fids:calloc");
...
static int cmd_del( void )                     /* delete the actual file       */
{
	...
	if ( !stat_setact )  stat_setact = setact('.');
        save_cursor();
	gotoxy(1,TermLines-1);
	...
	else
	{
	    ...
	    {    prt_str(0,TermCols-21,p_ls[acti]->f_name);
		 prt_str(0,3,"...");    }
	    else prt_str(0,TermCols-18,p_ls[acti]->f_name);
...
int sig_num;
{
	...
	switch( sig_num )
	{
	    ...
	    default :
		fprintf(stderr, " ==> %sSignal #%d%s <== : Tschuess!",
					A_FAILURE,sig_num,A_NORMAL);
		cmd_exit();
	}
	return;
}
...
static int cmd_rel( void )                     /* print copyright and release  */
{
	...
	printf("\t%sF%sullscreen %sI%snteractive %sD%sirectory %sS%shell\r\n",
	    A_TITLE,A_NORMAL,A_TITLE,A_NORMAL,A_TITLE,A_NORMAL,A_TITLE,A_NORMAL);
	printf("\t======================================\r\n");
	printf("\tVersion: %s, Release: %s,    date: %s\r\n\n",VERSION,RELEASE,GEN_DAT);
...
char what;
{
	...
	switch( what )
	{
	    case 'c':   cls();              /* read new and make screen */
			makescreen();
                        /* check the terminal lines and cols */
                        if( TermLines < TERM_MIN_LINES || TermCols < TERM_MIN_COLS )
                        {
	    ...
	    case 'b':                           /* read actual dir new  */
			if (actlr == 'r') { actir = acti; scrr = scr; }
			else              { actil = acti; scrl = scr; }
			...
			list_pwd = what;
			scr = 0; acti = 1;
			for( i = 0; i < numd && i < (TermLines-FLINES)*FileLstRows/scrn; i++ )
			{                         /* search the dirname */
