...
void listfiles(char what)
{
    ...
    switch (what)
    {
        ...
        case 'b':
            ...
            if (scrn == 1)
            {
                ...
                if ((l_pwd = strlen(p_cwd)) <= (TermCols - (FrightWinCols + 1)))
                    prt_str(-(TermCols - (FrightWinCols + 1)),
                            TermCols - (FrightWinCols + 1), p_cwd);
                else
                {
                    prt_str(0, 3, "...");
                    prt_str(0, TermCols - (FrightWinCols + 1) - 3,
                            &p_cwd[l_pwd - (TermCols - (FrightWinCols + 1) - 3)]);
                }
                printf(A_NORMAL);
            ...
            else
            {
                ...
                printf(A_WORKDIR);
                if ((l_pwd = strlen(p_cwd)) <=
                    (FileLstNameLen + 1) * FileLstRows / 2 + 1)
                    prt_str(-((FileLstNameLen + 1) * FileLstRows / 2 + 1),
                            (FileLstNameLen + 1) * FileLstRows / 2 + 1, p_cwd);
                else
                {
                    prt_str(0, 3, "...");
                    prt_str(0, (FileLstNameLen + 1) * FileLstRows / 2 - 2,
                            &p_cwd[l_pwd -
                                   ((FileLstNameLen + 1) * FileLstRows / 2 - 2)]);
                }
                printf(A_NORMAL);
                ...
                {
                    if (actlr == 'r')
                        printMountInfo((FileLstNameLen + 1) * FileLstRows / 2 +
                                           FileLstNameLen * FileLstRows / 4 + 5,
                                       i);
                    else
        ...
        case '+':
            if ((scr + (TermLines - FLINES) * rows) < num)
            {
        ...
        case '-':
            if ((scr - (TermLines - FLINES) * rows) >= 0)
            {
                scr -= (TermLines - FLINES) * rows;
                if (scr == 0)
                    acti = 1;
    ...
    for (x = 0; x < rows; x++) /* loop for each row    */
    {
        for (i = 0; i < (TermLines - FLINES); i++)
        {
            if (actlr == 'l') /* for scrn == 1 || 2   */
                gotoxy(x * (FileLstNameLen + 1) +
                           (FileLstRows == 2 ? x + 2 : 2) + x / 2,
                       i + 3);
            if (actlr == 'r') /* scrn must be 2       */
                gotoxy((FileLstRows == 2 ? 1 : x + 2) * (FileLstNameLen + 1) +
                           3,
                       i + 3);
            ...
            else
            {
                if (scrn == 1 || what == 'c')
                    ...
                    prt_str((FileLstNameLen + 1), (FileLstNameLen + 1), " ");
            } /* if */
...
leave:
    if (actlr == 'l')
        ...
        gotoxy((FileLstNameLen + 1) * FileLstRows / 2 + 6, 2);
...
void printMountInfo(int icols, int max_len)
{
    ...
    {
        if (FileLstRows == 4 && (i == FileLstNameLen + 3 /* 17 */ ||
    ...
    // from the mig2nto lib: fsys_get_mount_dev
    {
        ...
        if (_connect_combine(p_cwd, 0, O_ACCMODE, SH_DENYNO, 0, _FTYPE_ANY,
                             sizeof(msg.ii), &msg.ii, sizeof(msg.oo),
                             &msg.oo) == -1)
            return;
...
#endif
            strncat(fmtstr, p_fstype, min(sizeof(fmtstr) - 7, max_len - 6));
