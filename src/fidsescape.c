...
static int PF_load(char *l_path) /* load PF_keys from the file l_path */
{
    ...
    if (fgets(buff, LINE_LEN - 1, fp) == NULL)
    {
        fclose(fp);
        if (how)
            error("unexpected end of file ! : ", l_path);
        else
            *l_path = '\0';
        return (-1);
    }
                ...
                if (*z >= '1' && *z <= '9' && *(z + 1) == '=')
                {
                    switch (*(z - 1))
                    {
                        ...
                        case 'H': /* ".Hn=" with n 1..9  for HISTORY strings */
                            ...
                            p1 = pn = p_cmdl[(*z) - '0'];
                            for (z += 2, n = CMDL_C; isprint(*z) && n > 0;
                                 z++, p1++, n--)
                            {
                                *p1 = *z;
                            }
                ...
                else if (*z >= '@' && *z <= '_' && *(z + 1) == '=')
                {
                    switch (*(z - 1))
                    {
                        ...
                        case 'D':
                            ...
                            {
                                case 'I':
                                    p1 = filespec; /* incl. filespec */
                                    break;
                                case 'E':
                                    p1 = exclspec; /* excl. filespec */
                                    break;
                                case 'L':
                                    p1 = dir_n_l; /* dir_name_left */
                                    break;
                                case 'R':
                                    p1    = dir_n_r; /* dir_name_right */
                                    scrn  = 2;
                                    actlr = 'l'; /* .DA should follow ! */
                                    break;
                                case 'S':
                                    p1 = dir_n_s; /* dir_name_saved */
                                    break;
                            }
                            if (p1 != NULL)
                            { /* do the directory name copy */
                                for (z += 2, n = NDIR; isprint(*z) && n > 0;
                                     z++, p1++, n--)
                                {
                                    *p1 = *z;
                                }
...
int c;
{
    ...
    if (c == ESC) /* one single Escape Key */
    {
        ...
        printf(" %sh%s%selppage%s  %sl%s%soad pf_keys%s", A_ESCHELPKEY,
               A_NORMAL, A_ESCHELP, A_NORMAL, A_ESCHELPKEY, A_NORMAL, A_ESCHELP,
               A_NORMAL);
        printf("  %s dir:%s %ss%s%save%s %sr%s%sestore%s", A_ESCHELPKEY,
               A_NORMAL,                                     /* dirs */
               A_ESCHELPKEY, A_NORMAL, A_ESCHELP, A_NORMAL,  /*save*/
               A_ESCHELPKEY, A_NORMAL, A_ESCHELP, A_NORMAL); /*restore*/
        printf("  %s change file:%s %sn%s%same%s %su%s%sid%s %sg%s%sid%s "
               "%sm%s%sode%s %s?%s%s",
    ...
    switch (c)
    {
        ...
        case 'n':
            ...
            sbe[1] = '\0';
            if (*sbe)
                sprintf(p_cmdl[0], "mv %c%s%c %c", *sbe, p_ls[acti]->f_name, *sbe,
                        *sbe);
            else
        ...
        case 'm':
            ...
            if (c == 'u')
                ...
                sprintf(p_cmdl[0], "chmod %04o ", p_ls[acti]->f_st.st_mode & 07777);
            if (*sbe)
                strcat(p_cmdl[0], sbe);
            strcat(p_cmdl[0], p_ls[acti]->f_name);
            if (*sbe)
