...
int cmd_view()                        /* command execution for view     */
{
	    ...
	    ( (p_ls[acti]->f_st.st_mode & S_IFMT) != S_IFREG ) )
	{
	    ...
	    {    prt_str(0,30,p_ls[acti]->f_name);
		 prt_str(0,3,"...");    }
	    else prt_str(0,33,p_ls[acti]->f_name);
	...
	if( *(p_ls[acti]->f_name) != '\0' )
	{
          ...
          { /* Shared memory in QNX */
            ...
            if( fd != -1 )
            {
              ...
              madr = (char*)mmap( 0, p_ls[acti]->f_st.st_size,
	...
	for (;;)
	{
		...
		for( ;; p++ )
		{
			...
			if ( !typeahead )
			{
			    ...
			    if ( !(c_line % 8) )
			    {
				if ( cready() ) typeahead=TRUE;
			    }
			}
			if ( *p == '\t' ) c_line += 8-c_line%8;
			else            c_line++;    /* char's per line */
			if ( c_line < TermCols )
				continue;
		    }
		    c_line = 0;                         /* char is a CR */
		    line++;                          /* number of lines */
		    if ( !typeahead && cready() ) typeahead=TRUE;
			...
			else if ( !typeahead )
			{
			    ...
			    {
				gotoxy(46,1);
				printf("(%2d%%)",(offset+(long)(p+1-buf_beg))*100/f_size);
			    }
			}
			fflush(stdout);
			typeahead=FALSE;
		...
		for (j=0; j < line; j++)
		{
		    ...
		    if ( !typeahead ) printf("%06X    ",offset-rest + j*16);

		    if (!j) gotoxy( (rest/2) + (rest*2) + 11, 2);
		    z = p;                      /* save p in z          */
