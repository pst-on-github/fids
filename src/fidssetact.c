...
void printdate(p_tm) struct tm *p_tm;
{
    static char *month[] = {
        "Jan",
        "Feb",
        ...
        "Nov",
        "Dec"};
    printf("%s,%02d %02d", month[p_tm->tm_mon], p_tm->tm_mday,
           p_tm->tm_year % 100);
}
...
char    what;
{
    ...
    switch (what)
    {
        ...
        case 'B':
            if (acti < num - 1)
                nact++;
            if (nact >= rows * (TermLines - FLINES) + scr)
                listfiles('+');
            break;
        case 'C':
            if ((acti == rows * (TermLines - FLINES) + scr - 1) &&
                (acti < num - 1))
            { /* lower right corner */
                nact++;
                listfiles('+');
            ...
            if ((nact += (TermLines - FLINES)) > num - 1)
                nact = num - 1;
            if (nact >= rows * (TermLines - FLINES) + scr)
                nact = rows * (TermLines - FLINES) + scr - 1;
            break;
        case 'D':
            if ((acti == scr) && (acti > 0))
            { /* upper left corner */
    ...
    gotoxy(row * (FileLstNameLen + 1) + (FileLstRows == 2 ? row + 3 : 3) +
               row / 2,
           (acti - scr) % (TermLines - FLINES) + 3);
    ...
    if (what == 'S') /* old position underlined      */
    {
        ...
        if (actlr == 'l')
            ...
            gotoxy((FileLstNameLen + 1) * FileLstRows / 2 + 3, 2);
    ...
    *SymLinkName = '\0';
    ...
    if ((p_ls[acti]->f_st.st_mode & S_IFMT) == S_IFLNK)
    {
        ...
        {
            SymLinkName[numrdl] =
                '\0'; /* readlink in 'SCO' don't terminate the string ! */
    ...
    *p = '\0';
    ...
    if ((pw = getpwuid(p_ls[acti]->f_st.st_uid)) == NULL)
        ...
        sprintf(act_uid, "%.16s", pw->pw_name); /* file_uid     */
    ...
    if ((gr = getgrgid(p_ls[acti]->f_st.st_gid)) == NULL)
        ...
        sprintf(act_gid, "%.16s", gr->gr_name); /* file_gid     */
    printf(A_ACTFILEDATA);
...
void printFileSystemInfo(void)
{
    ...
    if (STATFS_BLOCK_TOTAL > 0)
        freepro =
            (int)((float)STATFS_BLOCK_FREE * 100. / (float)STATFS_BLOCK_TOTAL);
    else
        freepro = 0;
    if (freepro > 99)
        freepro = 99;
    ...
    printf("%s%cB=%s%2d%s%%\n", A_NORMAL, freec, A_ACTFILEDATA, freepro,
           A_NORMAL);
